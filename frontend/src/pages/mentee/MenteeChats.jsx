...
export default function MenteeChats() {
  const { getMyRequests } = useUserService();
          ...
          <>
            ...
            <div className="p-4 md:px-8 border-b border-slate-800 flex justify-between items-center bg-slate-900/30">
              <div className="flex items-center gap-3">
                ...
                <span className="font-black text-white italic uppercase text-xs md:text-sm tracking-widest truncate max-w-[150px] md:max-w-none">
                  {activeContact.Mentor?.user?.usr_name}
                </span>
              </div>
              <MoreVertical size={18} className="text-slate-600 cursor-pointer hover:text-white shrink-0" />
            </div>
              ...
              {messages.map((msg, idx) => {
                ...
                return (
                  <div key={idx} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-[85%] md:max-w-[70%] p-4 rounded-2xl text-sm leading-relaxed ${isMe ? 'bg-indigo-600 text-white rounded-tr-none' : 'bg-slate-900 text-slate-200 border border-slate-800 rounded-tl-none'}`}>
                      {msg.content}
